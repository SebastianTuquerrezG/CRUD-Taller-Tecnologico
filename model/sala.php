                ...
                <div class="body text-center container-fluid row justify-content-center align-items-center">
                    <div class="col-12 p-4 table-container">
                        <table class="table">
                            ...
                            <tbody>
                                ...
                                $sql = $conexion->query("select salas.id as idsala, salas.nombresala, sedes.nombresede from salas join sedes on salas.idsede = sedes.id");
                                while($datos = $sql->fetch_object()){ ?>
                                    <tr>
                                        <td><?= $datos->nombresala ?></td> 
                                        <td><?= $datos->nombresede ?></td> 
                                        <td>
                                            <a class="btn btn-small btn-warning" data-bs-toggle="modal" data-idsala="<?= $datos->idsala ?>" data-nombresala="<?= $datos->nombresala ?>" data-nombresede="<?= $datos->nombresede ?>" data-bs-target="#actualizarModal"><i class="fa-solid fa-pen-to-square"></i></a>
                                            <a class="btn btn-small btn-danger" data-bs-toggle="modal" data-idsala="<?= $datos->idsala ?>" data-bs-target="#eliminarModal" data-nombresala="<?= $datos->nombresala ?>" data-nombresede="<?= $datos->nombresede ?>"><i class="fa-solid fa-trash"></i></a>
                                        </td>
                                    </tr>
                                <?php }
                                ?>
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form method=POST action="../controller/registro_sala.php">
                                        <label for="codigo" class="form-label">Nombre de la Sala</label>
                                        <input type="text" class="form-control mb-3" id="codigo" name="nombresala">
                                        <label for="codigo" class="form-label">Sede de la Sala</label>
                                        <select class="form-control mb-3" id="tipo" name="nombresede">
                                            <?php
                                            include "../conexion/conexion.php";
                                            $sql = $conexion->query("select * from sedes");
                                            while($datos = $sql->fetch_object()){ ?>
                                                <option id="nombresede"><?= $datos->nombresede ?></option>
                                            <?php }
                                            ?>
                                        </select>
                                        <button type="submit" class="btn btn-primary" value="ok" name="btnregistrarsala">Registrar</button>
                                    </form>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="modal fade" id="actualizarModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="exampleModalLabel">Actualizar Sala</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    <form method="POST" action="../updates/actualizar_sala.php">
                                        <label for="codigo_actualizar" class="form-label">Nombre de la Sala</label>
                                        <input for="id_sala_actualizar" style="display: none;" id="id_actualizar" name="idsala"></input>
                                        <input type="text" class="form-control mb-3" id="codigo_actualizar" name="nombresala">
                                        <label for="codigo" class="form-label">Sede de la Sala</label>
                                        <select class="form-control mb-3" id="nombresede" name="nombresede">
                                            <?php
                                            include "../conexion/conexion.php";
                                            $sql = $conexion->query("select * from sedes");
                                            while($datos = $sql->fetch_object()){ ?>
                                        ...
                                        <button type="submit" class="btn btn-primary" value="ok" name="btnregistrarsala">Actualizar</button>   
                                    </form>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="modal fade" id="eliminarModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <p>¿Estás seguro de que deseas eliminar la sala "<span id="sala_a_eliminar"></span>"?</p>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                                    <form id="form_eliminar" method="POST" action="../delete/eliminar_sala.php">
                                        <input type="hidden" id="id_sala_eliminar" name="idsala">
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            actualizarLinks.forEach(function (link) {
                link.addEventListener('click', function () {
                    var nombresala = this.getAttribute('data-nombresala');
                    var nombresede = this.getAttribute('data-nombresede');
                    var idsala = this.getAttribute('data-idsala');
                    var inputSala = document.getElementById('codigo_actualizar');
                    var inputSede = document.getElementById('nombresede');
                    var inputId = document.getElementById('id_actualizar');
                    inputSala.value = nombresala;
                    inputSede.value = nombresede;
                    inputId.value = idsala;
                });
            });
        });
    </script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            eliminarLinks.forEach(function (link) {
                link.addEventListener('click', function () {
                    var nombresala = this.getAttribute('data-nombresala');
                    var idsala = this.getAttribute('data-idsala');
                    var spanSala = document.getElementById('sala_a_eliminar');
                    var inputId = document.getElementById('id_sala_eliminar');
                    spanSala.textContent = nombresala;
                    inputId.value = idsala;
                });
            });
        });
